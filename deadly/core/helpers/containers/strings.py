class Strings(object):
    ...
    def about_tab_string(self):
        ...
        text += "I am Deadly made by 【𓆩𝘽𝙇𝘼𝙕𝙀 ✘ 𝙊𝙋𓆪】⛓️ᥫʀ᭡.\n"
        ...
        text += "which are related to deadlyuserbot then just ask in\n"
        text += f"[support group](https://t.me/thedeadlybots)"
    ...
    def close_tab_string(self):
        ...
        text += "which will help you to understand  operate your userbot & assistant ( Deadly )\n"
        text += "\n\n• Menu is closed"
    ...
    def ialive_tab_string(self):
        ...
        text += f"**⟜ Deadly:** `{self.userbot_version}`\n"
        text += f"**⟜ Python:** `{self.python_version}`\n"
        text += f"**⟜ Pyrogram:** `{self.pyrogram_version}`\n"
        text += f"**⟜ uptime:** `{self.uptime()}`\n"
