...
async def sendplugin_handler(_, m: Message):
    ...
    if app.long() > 1:
        ...
        modulename = f"deadly/modules/userbot/{filename}.py"
...
async def install_handler(_, m: Message):
    ...
    if reply:
        ...
        module_loc = (
            f"./deadly/modules/userbot/{doc_name}"
        )
    ...
    try:
        if app.long() > 1:
            if cmd[1].endswith(".py"):
                module_loc = f"./deadly/modules/userbot/{cmd[1]}"
            elif not cmd[1].endswith(".py"):
                module_loc = f"./deadly/modules/userbot/{cmd[1]}.py"
            if os.path.exists(module_loc):
